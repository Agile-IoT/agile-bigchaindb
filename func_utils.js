 ...
 */
function Ed25519Keypair(passcode) {

    var buf = new ArrayBuffer(passcode.length*4); // 2 bytes for each char
    var bufView = new Uint8Array(buf);
    var c = 0;
    for (var i=0, strLen=passcode.length; i<strLen; i++) {
        bufView[c] = passcode.charCodeAt(i);
        c=c+2;
    }

    var keyPair = _tweetnacl2.default.sign.keyPair.fromSeed(bufView.slice(0,32));
...
/**
 * Create Ed25519 Cryptocondition to put into an Output of a Transaction
 * @param {string} publicKey base58 encoded public key for the recipient of the Transaction
